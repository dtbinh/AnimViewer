...
template<> void parse_elem< float >( const TiXmlElement* elem, float* d )
{
    parse_attrib_float( elem, true, "v", d );
}
...
template<> void parse_elem< Vector2 >( const TiXmlElement* elem, Vector2* vector )
{
    ...
    parse_attrib_float( elem, true, "u", &vector->x );
    parse_attrib_float( elem, true, "v", &vector->y );
}

template<> void parse_elem< Vector3 >( const TiXmlElement* elem, Vector3* vector )
{
    parse_attrib_float( elem, true, "x", &vector->x );
    parse_attrib_float( elem, true, "y", &vector->y );
    parse_attrib_float( elem, true, "z", &vector->z );
}
...
template<> void parse_elem< Quaternion >( const TiXmlElement* elem, Quaternion* quat )
{
    real_t x,y,z; // axis
    real_t a;     // angle
    parse_attrib_float( elem, true, "a", &a );
    parse_attrib_float( elem, true, "x", &x );
    parse_attrib_float( elem, true, "y", &y );
    parse_attrib_float( elem, true, "z", &z );
